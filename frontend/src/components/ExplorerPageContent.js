...
export default function ExplorerPageContent() {
  ...
  const filteredCountries = useMemo(() => {
    const filtered = enrichedCountries
      .filter((country) =>
      ...
      .filter(
        (country) =>
          selectedWorldCups === "" ||
          country.footballInfo?.worldCups === Number.parseInt(selectedWorldCups, 10)
      );
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            ...
            <p className="max-w-2xl text-sm leading-6 text-slate-300">
              Consulta informacion oficial de cada pais y complementala con un contexto
              futbolistico local para enriquecer la experiencia del usuario.
            </p>
          </div>
          ...
          <div className="w-full max-w-xl space-y-4">
            <div className="flex gap-4">
              ...
              <select
                ...
                onChange={(event) => setSelectedWorldCups(event.target.value)}
                className="rounded-lg border border-white/15 bg-slate-800 px-4 py-2 text-white focus:border-emerald-400 focus:outline-none"
              >
