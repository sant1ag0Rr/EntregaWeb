...
export default function ComparisonPanel({ countries, onRemove, onRandomCompare }) {
  return (
    <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 shadow-panel">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-4">
          <button
            type="button"
            onClick={onRandomCompare}
            className="rounded-lg border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-sm font-semibold text-emerald-200 transition hover:bg-emerald-400/20 hover:text-emerald-100"
          >
            Comparar aleatorios
          </button>
